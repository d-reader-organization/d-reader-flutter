...
class CandyMachineRepositoryImpl implements CandyMachineRepository {
  ...
  Future<String?> constructNftTransaction({
    ...
    return await client
        .get(
            '/candy-machine/transactions/mint-one?candyMachineAddress=$candyMachineAddress&minterAddress=$minterAddress&label=')
        .then(
          (value) => value.data,
        );
  }
  ...
  Future<String> useComicIssueNftTransaction(
    ...
    try {
      final transaction = await client
          .get(
            '/candy-machine/transactions/use-comic-issue-nft?nftAddress=$nftAddress&ownerAddress=$ownerAddress',
          )
          .then((value) => value.data);
      return transaction;
